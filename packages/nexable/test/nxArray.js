describe("nx.array", function () {
    ...
    it("Should calculate sum (reducer) correctly", function () {
        var inst = nx.array([1, 2, 3, 4, 5]);
        expect(inst.sum()).to.equal(15);
        inst.push(6);
        expect(inst.sum()).to.equal(15);
        nx.signal();
        expect(inst.sum()).to.equal(21);
    });

    it("Should map correctly", function () {
        var inst = nx.array([1, 2, 3, 4, 5]);
        var resu = inst.map(function (x) { return x * 2; });
        expect(resu()).to.deep.equal([2, 4, 6, 8, 10]);
        inst.push(6);
        nx.signal();
        expect(resu()).to.deep.equal([2, 4, 6, 8, 10, 12]);
    });

    it("Should filter correctly", function () {
        var inst = nx.array([1, 2, 3, 4, 5]);
        var resu = inst.filter(function (x) { return x > 2; });
        expect(resu()).to.deep.equal([3, 4, 5]);
        inst.push(6);
        nx.signal();
        expect(resu()).to.deep.equal([3, 4, 5, 6]);
    });
});
