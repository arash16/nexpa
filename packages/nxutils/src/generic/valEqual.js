...
function valEqual(oValue, nValue, update) {
    var oldValue = toPrimitiveValue(oValue),
    ...
    if (update == 'deep' && isObject(oldValue) && isObject(newValue)) {
        var oKeys = getObjectKeys(oldValue),
            kLen = oKeys.length,
            pairs;
        ...
        for (i = 0; i < kLen; i++) {
            ...
            if (isObject(ov)) {
                var nv = toPrimitiveValue(newValue[key]);
